...
struct node* del_list(struct node *head){  // delete all linked list 
    struct node *temp=head;
    while(temp!=NULL){
        temp=temp->link;   // point to next link
        free(head);       // delete head
        head=temp;        //updating head value
    }
   return head;
}
int main() {
 ...
 /* add_beg(head,89);          //use when adding at begginng
  ...
  head = del_list(head);    // delete all list 
  if(head==NULL){
    printf("\nlist is deleted");
  }
    return 0;
